#!/usr/bin/env bash
# Sequentially train the 80 km/h v3 BDP-Frenet and builtin PPO pair (25 candidates).
...
echo "Starting 80 km/h MetaDrive Frenet-PID v3 BDP-Frenet training."
...
echo "Starting 80 km/h MetaDrive Frenet-PID v3 builtin PPO training."
"${BDP_BENCHMARK_PYTHON_BIN}" scripts/train.py \
  --config scripts/job_scripts/benchmark/metadrive_frenet_pid_v3_builtin_benchmark.yaml \
  "$@"
