...
# Run the 12 m/s capped builtin comparison from this bundle directory:
...
echo "Starting 12 m/s MetaDrive Frenet-PID v2 builtin PPO training."
"${BDP_BENCHMARK_PYTHON_BIN}" scripts/train.py \
  --config scripts/job_scripts/benchmark_metadrive_12mps/metadrive_frenet_pid_v2_builtin_benchmark.yaml \
  "$@"

echo "Starting 12 m/s MetaDrive native builtin PPO training."
"${BDP_BENCHMARK_PYTHON_BIN}" scripts/train.py \
